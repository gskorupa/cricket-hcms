...
public class DocumentApi {
    ...
    @APIResponseSchema(
        value = List.class,
        responseDescription = "List of document objects.",
        responseCode = "200"
    )
    @Operation(
        summary = "List documents",
        description = "List documents with the specified path. If the result is a single document and it is a binary file, the file will be returned as a download."
    )
    public Response getDocs(
        ...
        @Parameter(
            description = "If true, documents will be listed with their content.",
            required = false,
            example = "true",
            schema = @Schema(type = SchemaType.BOOLEAN)
        ) @QueryParam("content") boolean withContent,
        @Parameter(
            description = "Path to the document or directory. If not provided, the root directory will be listed.",
            required = false,
    ...
    @APIResponseSchema(
        value = List.class,
        responseDescription = "List of document paths.",
        responseCode = "200"
    )
    @Operation(
        summary = "List paths",
        description = "List paths of documents in the specified site."
    )
    ...
    @APIResponseSchema(
        value = List.class,
        responseDescription = "List of site names.",
        responseCode = "200"
    )
    @Operation(
        summary = "List site names",
        description = "List site names provided by the document service."
    )
    ...
    @Operation(
        ...
        description = "Find documents with the specified path and tag (name:value). Path is optional."
    )
    public Response findDocs(
        ...
        @Parameter(
            description = "Path to the document or directory. Default \"/\"",
            required = false,
            example = "/documentation/",
            schema = @Schema(type = SchemaType.STRING)
        ) @QueryParam("path") String path,
        ...
        @Parameter(
            description = "Tag name to sort by.",
            required = false,
            example = "date",
            schema = @Schema(type = SchemaType.STRING)
        ...
        if (pair.length != 2) {
            return Response.status(Response.Status.BAD_REQUEST)
                .entity("Invalid tag definition (expecting name:value)")
                .build();
        }
        if (path == null || path.equals("")) {
            path = "/";
        }
        List<Document> list = documentPort.findDocuments(
            path,
            pair[0],
            pair[1],
            sort,
    ...
    @APIResponseSchema(
        ...
        responseDescription = "First document matching rules",
        responseCode = "200"
    )
    @Operation(
        summary = "Find first document",
        description = "Find documents with the specified path and tag (name:value). Path is optional."
    )
    public Response findFirst(
        ...
        Document doc = documentPort.findFirstDocument(
            path,
            pair[0],
            pair[1],
            sort,
    ...
    @APIResponseSchema(
        value = Document.class,
        responseDescription = "Document object.",
        responseCode = "200"
    )
    @Operation(
        summary = "Get document",
        description = "Get a document (JSON object) with the specified path. If the document is a binary file, the file will be returned as a download."
    )
    ...
    @APIResponseSchema(
        value = List.class,
        responseDescription = "List of document names.",
        responseCode = "200"
    )
    @Operation(
        summary = "Full text search",
        description = "Search documents with the specified text."
    )
    public Response searchDocs(
        ...
        @Parameter(
            description = "Document language code or * for all supported languages",
            required = false,
            example = "en",
            schema = @Schema(type = SchemaType.STRING)
    ...
    @APIResponseSchema(
        value = String.class,
        responseDescription = "Document saved.",
        responseCode = "200"
    )
    @Operation(
        summary = "Save document to the storage",
        description = "Save a document. The document must be provided in the request body."
    )
    ...
    private User getUser(String token) {
        ...
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                logger.warn(
                    "Error getting user. Code: " + response.getStatus()
                );
            }
        } catch (Exception e) {
