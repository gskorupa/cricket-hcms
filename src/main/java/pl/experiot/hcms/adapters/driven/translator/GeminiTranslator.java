...
public class GeminiTranslator implements ForTranslatorIface {
    ...
    public Document translate(
        ...
        String model = (String) options.getOrDefault(
            "gemini.model",
            DEFAULT_MODEL
        );
        ...
        if (apiKey == null || apiKey.isEmpty()) {
            logger.error(
                "Gemini API key is required. Set gemini.api.key in options."
            );
            return null;
        }
        ...
        try {
            ...
            // Translate content
            String translatedContent = translateText(
                apiKey,
                model,
                prompt,
                options
            ...
            for (String fieldName : metadataFields) {
                if (
                    ...
                    document.metadata.containsKey(fieldName)
                ) {
                    String metadataValue = document.metadata.get(fieldName);
                    String metadataPrompt = buildTranslationPrompt(
                        metadataValue,
                        sourceLanguage,
                        targetLanguage
                    );
                    String translatedMetadata = translateText(
                        apiKey,
                        model,
                        metadataPrompt,
                        options
                    ...
                    if (translatedMetadata != null) {
                        translatedDocument.metadata.put(
                            fieldName,
                            translatedMetadata
                        );
                    }
            ...
            logger.debug(
                "Translated (gemini): " +
                    translatedDocument.content.substring(
                        0,
    ...
    private String translateText(
        ...
        try {
            ...
            // ObjectNode generationConfig = requestBody.putObject(
            //     "generationConfig"
            // );
            // generationConfig.put(
            //     "temperature",
            //     (Double) options.getOrDefault(
            //         "gemini.temperature",
            //         DEFAULT_TEMPERATURE
            //     )
            // );
            ...
            // // Add safety settings
            // requestBody
            //     .putArray("safetySettings")
            //     .addObject()
            //     .put("category", "HARM_CATEGORY_HARASSMENT")
            //     .put("threshold", "BLOCK_LOW_AND_ABOVE");

            String requestBodyString = objectMapper.writeValueAsString(
                requestBody
            );
            ...
            // Send the request
            // log the request
            logger.debug("Gemini API request: " + request.toString());
            ...
            // Parse the response
            logger.debug("Gemini API response: " + response.body());
            try {
